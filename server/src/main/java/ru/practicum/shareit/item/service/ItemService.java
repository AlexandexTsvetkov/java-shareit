...
public class ItemService {
    ...
    public ItemDto create(NewItemRequest newItemRequest, long userId) {
        ...
        if (newItemRequest.getRequestId() != null) {
            ...
            Optional<ItemRequest> itemRequest = itemRequestRepository.findById(newItemRequest.getRequestId());

            if (itemRequest.isEmpty()) {
                throw new NotFoundException(MessageFormat.format("Запрос вещи с id {0, number} не найден", newItemRequest.getRequestId()));
            }

            request = itemRequest.get();

        }

        Item newItem = ItemMapper.mapToItem(newItemRequest, user.get(), request);
